 ...
 */
function display_month(month, Event_list, day) {
    ...
    for (var _i = 0, _a = month.week_numbers; _i < _a.length; _i++) {
        ...
        for (var i = week_end_index - 7; i < week_end_index; i++) {
            if (day_array[i] !== undefined) {
                if (day_array[i] === (0, time_date_1.get_current_date)() && day_array[i] === day && is_current_month) {
...
function display_event(event) {
    ...
    console.log("Desciption: ".concat(event.description));
}
